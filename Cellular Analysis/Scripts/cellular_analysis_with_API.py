...
# If you need to check if files are nonexistent
with open('../Databases/unfounded.txt', 'r') as file:
...
counter = 1
for _, row in hospital_lat_long_df.iterrows():
    hospital_name = row['Hospital Name']
    latitude = row['Latitude']
    longitude = row['Longitude']
    
    # Optional check for selective api queries (tab accordingly)
    # if hospital_name in hospital_names and (pd.notna(row['Latitude']) or pd.notna(row['Longitude'])):
...
## To run individial API queries, use below code (replace fields as desired)
