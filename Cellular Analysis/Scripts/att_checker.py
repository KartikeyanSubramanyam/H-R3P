...
verizon_mcc_mnc_file = '../Databases/Verizon_MCC_MNC.csv'  # Replace with the path to your AT&T CSV file
verizon_df = pd.read_csv(verizon_mcc_mnc_file)
    ...
    if os.path.exists(file_path):
        ...
        for _, row in tower_data.iterrows():
            ...
            if not pd.isna(mcc) and not pd.isna(mnc):
                ...
                if not verizon_df[(verizon_df['mcc'] == mcc) & (verizon_df['mnc'] == mnc)].empty:
                    verizon_matches += 1
        
        # Step 9: Log results for this hospital
        results.append({
            'Hospital Name': hospital_name,
            'Verizon Towers': verizon_matches,
        })
    else:
...
results_df.to_csv('../Analysis_Results/hospital_verizon_tower_comparison.csv', index=False)
